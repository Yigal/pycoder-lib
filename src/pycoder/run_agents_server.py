#!/usr/bin/env python3
"""
Server runner for the PyCoder package.
...
def start_server(host="127.0.0.1", port=8000):
    """Start the FastAPI server with the agents endpoints."""
    import uvicorn
    # Load environment variables
    from dotenv import load_dotenv
    load_dotenv()
    uvicorn.run("pycoder.agents_server:app", host=host, port=port, reload=True)

def run_cli():
    """Entry point for the CLI command."""
    import argparse
    
    parser = argparse.ArgumentParser(description="Run the PyCoder server")
    parser.add_argument("--host", type=str, default="0.0.0.0", help="Host to run the server on (0.0.0.0 to allow connections from any IP address)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="PyCoder Server Runner")
