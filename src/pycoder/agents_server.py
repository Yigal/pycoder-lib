#!/usr/bin/env python3
"""
FastAPI server for the PyCoder package.

This module provides a REST API for interacting with AI agents
powered by various LLM providers including OpenAI, Anthropic, Gemini, Groq, and OpenRouter.
"""
...
from .agents import Agent, AgentRunner, get_model_api_keys, AgentResult, load_env_vars
...
# Path to agent definitions - use package data
PACKAGE_DIR = Path(__file__).parent
AGENT_DEFINITIONS_DIR = PACKAGE_DIR / "agents_definitions"
...
def start_server(host="0.0.0.0", port=SERVER_PORT, reload=False):
    """Start the FastAPI server."""
    load_env_vars()  # load environment variables from .env
    if reload:
        import uvicorn
        uvicorn.run("pycoder.agents_server:app", host=host, port=port, reload=reload)
    else:
